   ...
   <div class="box box-primary">
      <div class="box-body">
         ...
         <div id="content" style="overflow-x:auto;">
            <p><strong>Manage Subscription</strong></p>
            <table width="100%" id="table1" class="table-bordered table-striped table-hover table-condensed data-table all-table-design">
               <thead style="background:#00c0ef;color:#fff;">
                  <tr>
                     ...
                     <th>Description</th>
                     <th>Days</th>
                     <th>Yearly / Monthly Plan</th>
                     <th>Price / Offer(%)</th>
                     <!-- <th>Document Quantity</th> -->
                     <th>Status</th>
                     <th>Action</th>
                  </tr>
               </thead>
               <tbody>
                       @foreach($records as $key => $record)
                            <tr>
                                ...
                                <td>{{ $record->title}}</td>
                                <td>                        
                                    @foreach($record->categories as $v)
                                       <label class="badge badge-success">{{ $v->name }}</label><br>
                                    @endforeach
                                 </td>
                                <td>{!! $record->description !!}</td>
                                <td>{!! $record->days !!}</td>
                                <td>{!! $record->type !!} / {!! $record->monthly !!}</td>
                                <td>{!! $record->price !!} / {!! $record->offers !!}</td>
                                <!-- <td>{!! $record->document_quantity !!}</td> -->
                                <td>
                                    @if($record->status == 1)
                                     <a href="{{ route('subscriptions.status', [$record->id]) }}" class="btn btn-success">Active</a>
                                    @else
                                    <a href="{{ route('subscriptions.status', [$record->id]) }}" class="btn btn-danger">Inactive</a>
                                    @endif
                                </td>
                                <td>
                                    @can('subscription-edit')
                                    <a href="{{ route('subscriptions.edit', [$record->id]) }}" class="btn btn-xs btn-success"><i class="fa fa-edit"></i></a>
                                    @endcan
                                    <form action="{{ route('subscriptions.destroy', [$record->id]) }}" method="POST" style="display: inline-block;">
                                        @method('DELETE')
                                        @csrf
