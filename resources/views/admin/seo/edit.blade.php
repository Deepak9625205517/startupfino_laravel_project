             ...
             <form action="{{ route('seo-pages.update', [$edit->id] ) }}" method="post" enctype="multipart/form-data">
                                ...
                                <option value="">Select Page</option>
                                 @foreach($pages as $page)
                                       @if (old('page_id') ==$page->id)
                                          <option value="{{$page->id }}" {{($edit->page_id == $page->id) ? 'selected': ''}}>{{ $page->title }}</option>
                                       @else
                                          <option value="{{$page->id }}" {{($edit->page_id == $page->id) ? 'selected': ''}}>{{ $page->title }}</option>
                                       @endif
                                 @endforeach                                
                  ...
                  <div class="col-md-12">
                     <div class="form-group">
                              ...
                              <textarea type="text" class="form-control ckeditor" id="meta_description" placeholder="Enter Meta Description" name="meta_description">{{{ old('meta_description', $edit->meta_description) }}}</textarea>
                  ...
                  <div class="col-md-12">
                     <div class="form-group">
                              ...
                              <textarea type="text" class="form-control ckeditor" id="meta_keyword" placeholder="Enter Meta Keyword" name="meta_keyword">{{{ old('meta_keyword', $edit->meta_keyword) }}}</textarea>
